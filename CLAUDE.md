...
- 교훈: **최근접 exemplar ≫ 평균 프로토타입**(재확인). **SupCon 헤드·맥락 concat·SAM·visual prompting·관계추론
  ...
  **artery↔vein은 못 가르는 게 아니라(색 AUC 0.77, DINO도 선형 0.76), exemplar 1-NN readout이 버리는 것**
  (중심 cos 0.88, 최근접이 부위 넘나듦) — exp 042 해석 정정. **조직-oracle Δ+6.4pp**(완벽조직 가정).
- **표현 축 후속 두 게이트는 음성(exp 046·047):** ⓐ **조직-인식 soft-gate readout(M-rep2)은 +6.4 못 실현**
  — 실제 조직헤드 65.5%로는 신호보다 오류 주입, exemplar가 *이미* DINO 암묵 조직축(0.76)을 써서 약한 명시
...
- **해상도 레버는 045에서 포화(exp 048):** α-가중(best α0.7 +0.25 6/10 노이즈)·더타이트 L128(−4.3)·CLS(−4.9)·
  ...
  재확인(frozen이 이미 분리가능 신호를 다 씀, contrastive head는 region 구조 파괴). **LoRA(마지막 블록 백본
  reshape, exp 050)도 음성** — dev-CV 낙관(LoRA가 dev 전체 학습)은 +11.95/+11.71(10/10) "대박"처럼 보이나 **봉인
  test는 frozen 36.1 → LoRA 30.5/24.5**(더 나쁨), 17.6pp 간극이 곧 과적합(§2). **M-rep1 전체 음성(049 head +
  050 LoRA). 표현 축은 해상도(045) 외 소진.** (050은 봉인-test §1.7의 교과서적 검증 — 없었으면 +12를 돌파로 오인.)
  전 프로그램서 누수안전 천장을 움직인 건 둘뿐 — **데이터 스케일(041 +8.9/+10.1)·입력 해상도(045 +2.6)**.
